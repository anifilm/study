...
class Board extends CI_Controller {
	...
	public function view() {
		...
		$data['comment_list'] = $this->board_model->get_comment('ci_board', $this->uri->segment(3));
		// 작성자 검증용
		$data['writer'] = FALSE;
		...
		if ($this->session->userdata('logged_in') == TRUE) {
			// 해당 글의 작성자가 본인인지 검증
			$writer_id = $this->board_model->writer_check('ci_board', $this->uri->segment(3));
	...
	public function modify() {
		...
		if ($this->session->userdata('logged_in') == TRUE) {
			...
			if ($writer_id->username != $this->session->userdata('username')) {
				alert('본인이 작성한 글이 아닙니다.', '/board/view/'.$this->uri->segment(3));
				exit;
			}
