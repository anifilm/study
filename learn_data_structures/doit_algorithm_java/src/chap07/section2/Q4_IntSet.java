/* Q4
클래스 IntSet에 대해 집합 s1과 집합 s2의 교집합을 복사하는 메서드, 집합 s1과 집합 s2의 차집합을 복사하는 메서드를 작성하세요.
void intersectionOf(IntSet s1, IntSet s2) // 집합 s1과 s2의 교집합을 복사합니다.
void differenceOf(IntSet s1, IntSet s2)   // 집합 s1과 s2의 차집합을 복사합니다.
*/
...
public class Q4_IntSet {
    ...
    // 집합 s와의 합집합 구하기
    public boolean add(Q4_IntSet s) {
        boolean flag = false;
        for (int i = 0; i < num; i++) {
    ...
    // 집합 s와의 교집합 구하기
    public boolean retain(Q4_IntSet s) {
        boolean flag = false;
        for (int i = 0; i < num; i++) {
    ...
    // 집합 s와의 차집합 구하기
    public boolean remove(Q4_IntSet s) {
        boolean flag = false;
		for (int i = 0; i < num; i++) {
    ...
    // 집합 s의 부분집합인지 확인합니다.
    public boolean isSubsetOf(Q4_IntSet s) {
		for (int i = 0; i < num; i++) {
			int j = 0;
			for (; j < s.num; j++)
				if (set[i] == s.set[j])
    ...
    // 집합 s의 진부분집합인지 확인합니다.
    public boolean isProperSubsetOf(Q4_IntSet s) {
