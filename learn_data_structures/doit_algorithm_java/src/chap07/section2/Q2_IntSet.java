/* Q2
클래스 IntSet에 대해서 집합 s의 모든 요소를 추가하는 메서드 집합 s에 들어 있는 요소만 남기고 들어 있지 않은 요소는 삭제하는 메서드, 집합 s에 들어 있는 요소를 삭제하는 메서드를 추가하세요. 메서드의 실행에 따라 집합이 변경되는 경우에는 true를, 그렇지 않은 경우에는 false를 반환하도록 하세요.
...
public class Q2_IntSet {
    ...
    // 집합 s에 복사합니다.
    public void copyTo(Q2_IntSet s) {
    ...
    // 집합 s를 복사합니다.
    public void copyFrom(Q2_IntSet s) {
    ...
    // 집합 s와 같은지 확인합니다.
    public boolean equalTo(Q2_IntSet s) {
    ...
    public void unionOf(Q2_IntSet s1, Q2_IntSet s2) {
    ...
    // 집합 s와의 합집합 구하기
    public boolean add(Q2_IntSet s) {
        boolean flag = false;
        for (int i = 0; i < num; i++) {
    ...
    // 집합 s와의 교집합 구하기
    public boolean retain(Q2_IntSet s) {
        boolean flag = false;
        for (int i = 0; i < num; i++) {
    ...
    // 집합 s와의 차집합 구하기
    public boolean remove(Q2_IntSet s) {
        boolean flag = false;
		for (int i = 0; i < num; i++) {
